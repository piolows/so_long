...
/*   Updated: 2024/02/15 20:01:21 by ppolinta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	putactors2(t_data *data, t_tx tx, int *xy, int actor)
{
	...
	if (actor == 'R')
		mlx_put_image_to_window(data->mlx, data->win, tx.r.img, xy[0], xy[1]);
	if (actor == 'Q')
		mlx_put_image_to_window(data->mlx, data->win, tx.q.img, xy[0], xy[1]);
}
